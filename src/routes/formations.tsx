...
function CallButton({ className }: { className?: string }) {
  return (
    <>
      <style>{`
        #contact-modal-toggle { display: none; }
        #contact-modal-toggle:checked ~ .contact-modal-overlay { display: flex; }
        @media (max-width: 767px) {
          .contact-modal-label { display: none; }
          .contact-mobile-link { display: inline-flex; }
        }
        @media (min-width: 768px) {
          .contact-modal-label { display: inline-flex; }
      ...
      <input type="checkbox" id="contact-modal-toggle" />

      {/* Mobile : appel direct */}
      <a
        href="tel:+33978802232"
        className={`contact-mobile-link items-center gap-2 ${className}`}
      >
      ...
      <label
        htmlFor="contact-modal-toggle"
        className={`contact-modal-label items-center gap-2 cursor-pointer ${className}`}
      >
        Nous contacter <ArrowRight className="h-4 w-4" />
      </label>

      {/* Modal overlay */}
      <label htmlFor="contact-modal-toggle" className="contact-modal-overlay">
        <div
